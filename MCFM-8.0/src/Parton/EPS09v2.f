...
C Read the table
      ...
      OPEN (openchannel,file='Pdfdata/'//filenimi,
     1 status='OLD',IOSTAT=iovar)
...
***********************
      ...
      Subroutine EPS09_MC(order, pset, AAA, xxx, QQQ,
     &                   ruv, rdv, ru, rd, rs, rc, rb, rg)

      Implicit none
      Double precision :: ruv, rdv, ru, rd, rs, rc, rb, rg, QQQ, xxx
      Double precision :: LSTEP, x, Q, Q2, 
     1     allvalues(1:1000,1:8,0:50,0:50)
c     1     allvalues(0:50,0:50,1:1000,1:8)
...
C *********************************************
      ...
      If (pset .LT. 1 .or. pset .GT. psetmax) then
         Write(*,*) 'Wrong set!'
c         Write(*,*) 'Central set: pset = 1' ! Nestor modified it, 10.01.2013
c         Write(*,*) 'Error sets : pset = 2...31' ! Nestor modified it, 10.01.2013
         Write(*,*) 'Only ', psetmax, ' replicas available!'
         Stop
      End If
...
C Read the table
      ...
      If (order .EQ. 1) then
        ...
        If (A < 10) Then
c        Write(filenimi,'("EPS09LOR_", I1)'), A ! Nestor modified it, 10.01.2013
        Write(filenimi,'("EPS09LOR_", I1,"_MC")'), A
        Else If (A < 100) Then
c        Write(filenimi,'("EPS09LOR_", I2)'), A ! Nestor modified it, 10.01.2013
        Write(filenimi,'("EPS09LOR_", I2,"_MC")'), A
        Else If (A < 1000) Then
c        Write(filenimi,'("EPS09LOR_", I3)'), A ! Nestor modified it, 10.01.2013
        Write(filenimi,'("EPS09LOR_", I3,"_MC")'), A
        End If

      Else

        If (A < 10) Then
c        Write(filenimi,'("EPS09NLOR_", I1)'), A ! Nestor modified it, 10.01.2013
        Write(filenimi,'("EPS09NLOR_", I1,"_MC")'), A
        Else If (A < 100) Then
c        Write(filenimi,'("EPS09NLOR_", I2)'), A ! Nestor modified it, 10.01.2013
        Write(filenimi,'("EPS09NLOR_", I2,"_MC")'), A
        Else If (A < 1000) Then
c        Write(filenimi,'("EPS09NLOR_", I3)'), A ! Nestor modified it, 10.01.2013
        Write(filenimi,'("EPS09NLOR_", I3,"_MC")'), A
        End If
      ...
      OPEN (openchannel, file = 'Pdfdata/'//filenimi, status='OLD', 
     1 IOSTAT=iovar)

c ! Nestor modified it, 10.01.2013
      If (iovar .NE. 0) Then
      Write(*,*) 'Missing file: ',filenimi
      stop
      End If
